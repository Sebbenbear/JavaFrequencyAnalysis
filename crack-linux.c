...
#define _GNU_SOURCE  //to make qsort work
...
void freeSubTexts(char **subTextStrings, int i) {
  for(int stringIndex = 0; stringIndex < i; ++stringIndex){
    free(subTextStrings[stringIndex]);  //check for memory leaks here TODO
  }
}
...
void initializeFrequencyTable(char *text, size_t text_length) {
  // Zero the table
...
int frequency_comparator(const void *a, const void *b) {
  ...
  char char1 = *((const char *)a); //cast to char
  char char2 = *((const char *)b);
  
  return frequencyOf(char2) - frequencyOf(char1);
}

void initializeSortedChars(char *sortedChars) {
  memcpy(sortedChars, ALPHABET, sizeof(char) * ALEN);
  
  //different mapping from mac-linux
  /*  void qsort_r(void *base, size_t nmemb, size_t size,
                  int (*compar)(const void *, const void *, void *),
                  void *arg);
  */

  //qsort_r(sortedChars, ALEN, sizeof(char), frequencyTable, frequency_comparator); //bsd version
  //qsort_r(sortedChars, ALEN, sizeof(char), frequency_comparator, frequencyTable); //bsd version
  qsort(sortedChars, ALEN, sizeof(char), frequency_comparator);
...
void processFile(char *text, int n) {
  ...
  for(int i = 1; i <= n; ++i){
    ...
    for(int stringIndex = 0; stringIndex < i; ++stringIndex){  //for each substring
      ...
      int frequencyTable[ALEN];  //take this out
      initializeFrequencyTable(currentSubtext, currentSubtext_length);
      
      //Now sort them based on frequency
      char sortedChars[ALEN+1];
      sortedChars[ALEN] = '\0';
      
      initializeSortedChars(sortedChars);
      ...
      //Now apply the most characters to the text
